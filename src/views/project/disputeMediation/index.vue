<template>
    <div class="app-container">
                ...
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="是否执行完毕" prop="executionCompletedFlag">
                            <el-select v-model="queryParams.params.executionCompletedFlag" placeholder="请选择是否执行完毕" clearable style="width: 100%">
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button type="info" plain icon="el-icon-upload2" size="mini" @click="$refs.importRef.open()" v-hasPermi="['project:disputeMediation:import']">导入</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="info" plain icon="el-icon-upload2" size="mini" @click="$refs.importByHDRef.open()" v-hasPermi="['project:disputeMediation:importByHD']">河东法院导入</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="warning" plain icon="el-icon-download" size="mini" @click="$refs.exportAllRef.open()" v-hasPermi="['project:disputeMediation:export']">总局导出</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="warning" plain icon="el-icon-download" size="mini" @click="$refs.exportRef.open()" v-hasPermi="['project:disputeMediation:export']">自收案件总局导出</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="warning" icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['project:disputeMediation:dataExport']">数据导出</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    type="primary"
            ...
            <el-table-column label="案件编号" align="center" prop="workOrderId" v-if="columns.find(s => s.label === '案件编号').visible" sortable="custom" width="130px">
                <template slot-scope="scope">
                    <el-tooltip
                        ...
                        class="item"
                        effect="dark"
                        :content="'调解员接案时间:' + scope.row.mediatorAcceptTime"
                        placement="right"
                    >
                    ...
                    <el-tooltip
                        ...
                        class="item"
                        effect="dark"
                        :content="'发送反馈单时间:' + scope.row.sendFeedbackTime"
                        placement="right"
                    >
            ...
            <el-table-column label="操作" align="center" width="130">
                <template v-slot="{ row }">
                    ...
                    <el-button
                        ...
                        @click="showMediatorAuditRecord(row)"
                    ...
                    <el-button
                        ...
                        @click="showVideoAuditRecord(row)"
                        v-if="row.videoAuditStatus && ($store.getters.userInfo.isDMVideoAuditor1 || $store.getters.userInfo.isDMVideoAuditor2)"
                    >
                        {{
                            (row.videoAuditStatus.split(',').includes(DM_VIDEO_AUDIT_STATUS.STATUS1) && $store.getters.userInfo.isDMVideoAuditor1) ||
                            (row.videoAuditStatus.split(',').includes(DM_VIDEO_AUDIT_STATUS.STATUS2) && $store.getters.userInfo.isDMVideoAuditor2)
                                ? '录像审核'
                                : '录像审核记录'
                        }}
                    </el-button>
                    ...
                    <div v-if="!!row.agreementId || ($store.getters.userInfo.isDMMediator && DM_STATUS.DM_STATUS4 === row.status && DM_AGREEMENT_STEP.SF !== row.agreementStep)">
                        <el-dropdown trigger="click" size="mini" @command="noticeBookSelect($event, row)">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item command="edit" v-if="isDMMediator(row) && DM_STATUS.DM_STATUS4 === row.status && DM_AGREEMENT_STEP.SF !== row.agreementStep">
                                    编辑协议书
                                </el-dropdown-item>
                    ...
                    >
                        <el-dropdown trigger="click" size="mini" @command="handleAgreement($event, row)">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <template v-hasPermi="['project:disputeMediation:handle']">
                                    <el-dropdown-item command="a" icon="el-icon-circle-plus" v-if="DM_STATUS.DM_STATUS10 === row.status && $store.getters.userInfo.isDMInstitutionHandle">
                                        新增履约登记
                                    </el-dropdown-item>
                                </template>
                    ...
                    >
                        <el-dropdown trigger="click" size="mini" @command="downloadFile($event, row)">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item command="a" icon="el-icon-download" v-if="SYS_YES_NO.sys_yes === row.deptAcceptMediate && (DM_STATUS.DM_STATUS3 === row.status || row.feedbackTime)">
                                    下载反馈单
                                </el-dropdown-item>
                    ...
                    <el-tooltip class="item" :disabled="!row.archiveRejectReason" effect="dark" :content="row.archiveRejectReason" placement="left">
                        <el-badge :is-dot="DM_ARCHIVE_STATUS.reject === row.archiveStatus" class="item" v-if="DM_STATUS.DM_STATUS10 === row.status && isDMMediator(row)">
                            <el-button size="mini" type="text" icon="el-icon-document-checked" @click="handleArchiveApply(row)" :disabled="DM_ARCHIVE_STATUS.apply === row.archiveStatus">
        ...
        <el-dialog title="满意度" :visible.sync="satisfactionOpen" width="420px" append-to-body :close-on-click-modal="false">
            <el-form :model="satisfactionForm" label-width="88px">
                <el-form-item label="满意度">
                    <el-input-number v-model="satisfactionForm.satisfactionScore" :min="0" :max="100" :precision="0" controls-position="right" style="width: 100%" placeholder="0-100" />
                </el-form-item>
            </el-form>
        ...
        <!-- 河东法院导入 -->
        <Import ref="importByHDRef" dialog-title="河东法院导入" upload-path="/project/disputeMediation/importByHD" :show-template-links="false" :show-hd-template-link="true" @callback="getList" />
