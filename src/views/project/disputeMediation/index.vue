        ...
        <el-row>
          <el-col :span="6">
            <el-form-item label="姓名" prop="name">
              <el-input v-model="queryParams.name" placeholder="请输入消费者姓名或代理人姓名" clearable @keyup.enter.native="handleQuery" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item label="工单状态" prop="status">
              <el-select v-model="queryParams.params.statusIn" placeholder="请选择工单状态" clearable style="width: 100%" multiple>
          ...
          <el-row>
            <el-col :span="6">
              <el-form-item label="证件类型" prop="certType">
                <el-select v-model="queryParams.certType" placeholder="请选择消费者证件类型或代理人证件类型" clearable style="width: 100%">
            ...
            <el-col :span="6">
              <el-form-item label="证件号码" prop="certNum">
                <el-input v-model="queryParams.certNum" placeholder="请输入消费者证件号码或代理人证件号码" clearable @keyup.enter.native="handleQuery" style="width: 100%" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="保险消费投诉事由分类" prop="insuranceComplaintType">
                <template slot="label">
                  <span style="font-size: 9px;">保险消费投诉事由分类</span>
                </template>
...
<!--        </template>-->
<!--      </el-table-column>-->
      <el-table-column label="调解员接案时间" align="center" prop="mediatorAcceptTime" width="140px" v-if="columns.find(s => s.label === '调解员接案时间').visible" sortable="custom">
        <template slot-scope="scope">
      ...
      <el-table-column label="盖章反馈天数" align="center" prop="stampedFeedbackAttachmentDay" v-if="columns.find((s) => s.label === '盖章反馈天数').visible">
        <template slot-scope="scope">
          <el-tag :type="scope.row.stampedFeedbackAttachmentTime ? 'success' : 'danger'" v-if="scope.row.stampedFeedbackAttachmentDay != null">{{ scope.row.stampedFeedbackAttachmentDay }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="130">
        <template v-slot="{ row }">
          <el-button size="mini" type="text" icon="el-icon-info" @click="handleDetail(row)">
            详情
          </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        status: null,
        signWay: null,
        params: { phone: null, createTime: null, statusIn: null }
      },
      // 列信息
      columns: [
        ...
        {label: `调解员接案时间`, visible: true},
        {label: `机构名称`, visible: true},
        {label: `调解员`, visible: false},
  ...
  created() {
    ...
    institutionList().then((response) => {
      ...
      if (this.$store.getters.userInfo.isDMEntryClerk) {
        // 新增时，如果是接线员，则过滤对应的机构
        this.deptOptionsAdd = this.handleTree(response.data.filter(dept => {
          return (this.$store.getters.userInfo.isDMCenterBankEntryClerk || this.$store.getters.userInfo.isDMCourtBankEntryClerk) && DEPT_TYPE.bankList.includes(dept.type)
  ...
  methods: {
    ...
    resetQueryParams() {
      ...
      this.queryParams.params = { phone: null, createTime: null, statusIn: null };
    },
    /** 重置按钮操作 */
    resetQuery() {
      this.resetQueryParams();
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm(
          '是否确认删除纠纷业务工单编号为"' + workOrderIds + '"的数据项？'
        )
        .then(function () {
