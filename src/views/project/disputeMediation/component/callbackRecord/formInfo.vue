<template>
  <div>
    <el-form :model="formInfo" :rules="rules" ref="callbackRuleForm" label-width="110px">
      ...
      <el-form-item label="回访事由" prop="cause">
        <el-input v-model="formInfo.cause" :placeholder="isEdit?'':'请输入回访事由'" :disabled="isEdit" maxlength="40" show-word-limit clearable/>
      </el-form-item>

      <el-form-item label="回访情况" prop="content">
        <el-input type="textarea" v-model="formInfo.content" :placeholder="isEdit?'':'请输入回访情况'" :disabled="isEdit" :autosize="{ minRows: 3}" maxlength="1000" show-word-limit clearable/>
      </el-form-item>
    </el-form>
  </div>
</template>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cause: [{required: true, message: '回访事由不能为空', trigger: 'blur'}],
