    ...
    <el-col :span="14">
      ...
      <div class="btn">
        <template v-if="!isEditing">
          <el-button type="primary" @click="startEdit" style="margin-right: 10px" v-if="
            isDMMediator() &&
            ((DM_STATUS.DM_STATUS3 === row.status && SYS_YES_NO.sys_yes === row.deptAcceptMediate) || DM_STATUS.DM_STATUS4 === row.status || DM_STATUS.DM_STATUS10 === row.status)
          ">
            修 改
          </el-button>
        </template>
  ...
  methods: {
    open(row, data) {
      ...
      this.formData = {
        ...
        // 机构类型
        deptType: this.row.deptType || null,
