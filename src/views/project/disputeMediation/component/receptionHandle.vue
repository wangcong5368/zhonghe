      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item
            ...
            :rules="[{ validator: this.validCertNum(this.form.deptContactCertType), trigger: 'blur' },{ required: this.SYS_YES_NO.sys_no !== this.form.deptAcceptMediate, message: '证件号码为必填项', trigger: 'blur' }]"
          >
            <el-input
      ...
      <el-row v-if="DEPT_TYPE.bankList.includes(form.deptType) || DEPT_TYPE.nonBankList.includes(form.deptType)">
        <el-col :span="12">
          <el-form-item
            ...
            :rules="[{ validator: this.validCertNum(this.CERT_TYPE.CERT_TYPE0), trigger: 'blur' },{ required: this.SYS_YES_NO.sys_no !== this.form.deptAcceptMediate, message: '经办人身份证号为必填项', trigger: 'blur' }]">
            <el-input
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // ],
        sex: [
          { required: true, message: "消费者性别为必填项", trigger: "blur" },
        ],
        age: [
        ...
        // ],
        deptAcceptMediate: [
          { required: true, message: "金融机构是否接受调解为必填项", trigger: "blur" },
        ],
        enforceAgreementType: [
          { required: true, message: "履约类型为必填项", trigger: "blur" },
        ],
        solutionAmount: [
          { required: true, message: "调解方案金额为必填项", trigger: "blur" },
        ],
        solution: [
          { required: true, message: "自查情况、调解方案及依据为必填项", trigger: "blur" },
        ],
      },
      // 常量
  ...
  methods: {
    ...
    reject() {
      this.loading = true;
      handleReject(this.form).then((response) => {
        this.loading = false;
