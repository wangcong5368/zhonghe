<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="myItemList" @selection-change="handleSelectionChange"
                    ...
                    <template slot-scope="scope">
                        <el-dropdown trigger="hover" @command="handleCommand">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <!-- 办理按钮，仅在特定条件下显示 -->
                                <el-dropdown-item :command="{ type: 'handle', row: scope.row }"
                                    v-if="scope.row.organizerStatus !== 3 && scope.row.organizerStatus !== 4 && (scope.row.status === 0 || scope.row.status === 1 || scope.row.status === 6)">
                                    <i class="el-icon-edit"></i> 办理
                                </el-dropdown-item>
        ...
        <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body :close-on-click-modal="false">
            <el-form label-width="130px" label-position="right">
                ...
                <el-form-item label="事项内容" prop="content">
                    <!-- <editor v-if="open" v-model="form.content" :min-height="192" :readOnly="true" :key="true ? 'view-editor' : 'edit-editor'" /> -->
                ...
                <el-row>
                    <el-col :span="24">
                        <div class="line-container">
                            <div class="line"></div>
                            <div class="text">以下为主办人和协办人填写</div>
                            <div class="line"></div>
                        </div>
                    </el-col>
                </el-row>
            </el-form>
            <el-form ref="organizerForm" :model="organizerForm" :rules="rules" label-width="100px">
                <el-form-item label="进度回复" prop="content">
                    <!-- <editor v-model="organizerForm.content" :min-height="192" /> -->
                    <el-input type="textarea" v-model="organizerForm.content" :rows="3" />
                    <quick-reply :onSelect="handleSelectPhrase" :superviseId="form.id"></quick-reply>
                </el-form-item>
                <el-form-item label="附件上传" prop="url">
                    <FileUpload @upload-success="handleUploadSuccess" ref="fileUpload" v-model="organizerForm.url" />
                </el-form-item>
                <el-form-item label="备注" prop="notes">
                    <el-input type="textarea" v-model="organizerForm.notes" :rows="3" />
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="handleForward" v-if="!isMainOrganizer">增加协办人</el-button>
                <el-button type="primary" @click="handleProcess">进度更新</el-button>
                <el-button type="primary" @click="handleProcessCompleted" v-if="!isMainOrganizer">事项办结</el-button>
        ...
        <rm-dialog ref="rmDialog"></rm-dialog>
...
<script>
import { getSupervise, addSupervise, updateSupervise, getProcessList, addProcess, addOrganizer, addProcessCompleted, getProgressList } from '@/api/oa/supervise';
import { listDeptTree, getSourceDict, getTypeDict, getUrgencyDict, getAllUser, getDeptUserTree } from '@/api/oa/publicApi';
...
export default {
    ...
    data() {
        return {
            ...
            form: {},
            // 主办人 or 协办人表单参数
            organizerForm: { content: '' },
            // 表单校验
            rules: {
                content: [{ required: true, message: '请输入进度回复', trigger: 'blur' }]
            },
            // 下载相关
    ...
    methods: {
        handleSelectPhrase(phrase) {
            this.organizerForm.content = this.organizerForm.content + phrase
        },
        handleForward() {
            this.$refs.rmDialog.openDialog(this.form.id);
        },
        ...
        handleProcess() {
            // 提交前校验表单规则
            this.$refs.organizerForm.validate(valid => {
                if (!valid) {
        ...
        cancel() {
            ...
            if (this.$refs.organizerForm) {
                this.$refs.organizerForm.resetFields();
            }
        },
        // 表单重置
        reset() {
        ...
        /** 修改按钮操作 */
        handleUpdate(row) {
            this.organizerIds = row.organizerIds || '';
            this.reset();
            const id = row.id || this.ids;
        ...
        initColumnSettings() {
            ...
            const savedSettings = localStorage.getItem('myItem-column-settings');
            if (savedSettings) {
                try {
        ...
        saveColumnSetting() {
            try {
                localStorage.setItem('myItem-column-settings', JSON.stringify(this.allColumns));
