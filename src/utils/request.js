                ...
                MessageBox.confirm('登录状态已过期，您可以继续留在该页面，或者重新登录', '系统提示', { confirmButtonText: '重新登录', cancelButtonText: '取消', type: 'warning' })
                    .then(() => {
                        ...
                        store.dispatch('LogOut').then(() => {
                            location.href = conf.server.publicPath + '/index';
                        });
                    })
                    .catch(() => {
                        isRelogin.show = false;
...
export function download(url, params, filename, config) {
    service.defaults.timeout = 10*60*1000;
            ...
            if (isBlob) {
                ...
                try {
                    ...
                    if (resText.trim().startsWith('{') || resText.trim().startsWith('[')) {
                        ...
                        // 如果不是JSON格式，可能是文件内容，直接下载
                        const blob = new Blob([data]);
                        saveAs(blob, filename);
                    }
                } catch (error) {
                    // 如果text()转换失败，说明是二进制文件，直接下载
                    const blob = new Blob([data]);
                    saveAs(blob, filename);
                }
            }
